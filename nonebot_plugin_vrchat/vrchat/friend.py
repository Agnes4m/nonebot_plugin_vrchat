...
from .utils import auto_parse_iterable_return, iter_pagination_func
...
async def get_friends(
    ...
    async def iterator(page_size: int, offset: int) -> List[LimitedUser]:
        return await cast(
            ...
            run_sync(api.get_friends)(offset=offset, n=page_size, offline=offline),
        )
...
async def get_all_friends(
    ...
    async for x in await get_friends(
        client,
        offline=False,
        page_size=page_size,
        offset=offset,
